...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1 space-y-4">
        <div class="card p-5 space-y-4">
            ...
            @endif
            <div>
                <p class="text-xs text-slate-500 mb-0.5">Artwork Süreci</p>
                        ...
                        @can('manualArtwork', $line)
                            @if(! $line->hasActiveArtwork() && ! $line->is_manual_artwork_completed)
                                <form method="POST" action="{{ route('order-lines.manual-artwork.store', $line) }}" class="mt-4 rounded-lg border border-emerald-100 bg-emerald-50/50 p-3">
                                    ...
                                    @enderror
                                    <div class="mt-3">
                                        <button type="submit" class="btn btn-secondary text-emerald-700 border-emerald-200 hover:bg-emerald-100">Manuel gönderildi olarak işaretle</button>
                                    </div>
                                </form>
                            @endif
                        @endcan
            ...
            @if(! auth()->user()->isSupplier())
                <div class="px-5 py-4 border-t border-slate-100">
                    <form method="POST" action="{{ route('orders.notes.store', $order) }}" class="flex gap-3">
                        @csrf
                        <div class="flex-1">
                            <textarea name="body" rows="2" class="input resize-none" placeholder="Not ekle…">{{ old('body') }}</textarea>
                            @error('body')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
