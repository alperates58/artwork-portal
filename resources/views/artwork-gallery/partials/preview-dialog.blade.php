...
<dialog id="{{ $dialogId }}" class="max-h-[92vh] w-[min(96vw,1440px)] max-w-none overflow-hidden rounded-[32px] border border-slate-200 p-0 shadow-2xl backdrop:bg-slate-950/60">
    <div class="flex h-[min(92vh,960px)] min-h-0 flex-col">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2">
                        <h3 class="break-words text-xl font-semibold text-slate-950 xl:text-2xl">{{ $artworkGallery->display_name }}</h3>
                        @include('artworks.partials.passive-gallery-badge', ['galleryItem' => $artworkGallery])
                    </div>
                    <p class="mt-2 text-xs font-medium text-slate-500 sm:text-sm">
        ...
        <div class="grid min-h-0 flex-1 gap-5 overflow-y-auto px-5 py-5 xl:grid-cols-[minmax(0,1.9fr)_310px]">
            ...
            <aside class="space-y-3 xl:max-h-full xl:overflow-y-auto xl:pr-1">
                <div class="rounded-[28px] border border-slate-200 bg-white p-4">
                    <dl class="space-y-3">
                        <div>
                            ...
                            <dd class="mt-1 font-mono text-sm font-semibold text-slate-900 sm:text-[15px]">
                        ...
                        @if($artworkGallery->stockCard?->stock_name)
                            <div>
                                ...
                                <dd class="mt-1 text-sm leading-6 text-slate-900">{{ $artworkGallery->stockCard->stock_name }}</dd>
                            </div>
                        @endif
                        <div>
                            <dt class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Kategori</dt>
                            <dd class="mt-1 text-sm text-slate-900">{{ $artworkGallery->category?->display_name ?? 'Kategorisiz' }}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-slate-900">{{ $artworkGallery->file_type_display }} · {{ $artworkGallery->file_type_description }}</dd>
                        </div>
                        <div>
                            <dt class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Yükleyen</dt>
                            <dd class="mt-1 text-sm text-slate-900">{{ $artworkGallery->uploadedBy->name }}</dd>
                        </div>
                        <div>
                    ...
                    @if($usageHistoryUrl)
                        <a href="{{ $usageHistoryUrl }}" class="mt-4 inline-flex items-center rounded-xl border border-brand-200 bg-brand-50 px-3 py-2 text-xs font-semibold text-brand-700 transition hover:border-brand-300 hover:bg-brand-100">
                            Sipariş Geçmişi
                        </a>
                    @endif
                </div>
                ...
                <div class="rounded-[28px] border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="mt-2 text-sm leading-5 text-slate-700">{{ $artworkGallery->display_revision_note ?: 'Not bulunmuyor.' }}</p>
                </div>
            </aside>
        </div>
    </div>
